...
public class EditorObjExporter : ScriptableObject
{
    ...
    static void ExportSelectionToSeparate()
    {
        ...
        for (int i = 0; i < selection.Length; i++)
        {
            ...
            for (int m = 0; m < meshfilter.Length; m++) { exportedObjects++; MeshToFile((MeshFilter)meshfilter[m], targetFolder, selection[i].name + "_" + i + "_" + m); }
        }
    ...
    static void ExportWholeSelectionToSingle()
    {
        ...
        for (int i = 0; i < selection.Length; i++)
        {
            ...
            for (int m = 0; m < meshfilter.Length; m++) { exportedObjects++; mfList.Add(meshfilter[m]); }
        }
        if (exportedObjects > 0)
        {
            MeshFilter[] mf = new MeshFilter[mfList.Count];

            for (int i = 0; i < mfList.Count; i++) { mf[i] = (MeshFilter)mfList[i]; }
            string filename = EditorApplication.currentScene + "_" + exportedObjects; int stripIndex = filename.LastIndexOf('/');//FIXME: Should be Path.PathSeparator                if (stripIndex >= 0)
    ...
    static void ExportEachSelectionToSingle()
    {
        ...
        for (int i = 0; i < selection.Length; i++)
        {
            ...
            for (int m = 0; m < meshfilter.Length; m++) { exportedObjects++; mf[m] = (MeshFilter)meshfilter[m]; }
